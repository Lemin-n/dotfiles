...
with vimPlugins;
{
  ...
  config = ''
    require("presence").setup({
    	...
    	neovim_image_text = "The True Text Editor", -- Text displayed when hovered over the Neovim image
    	...
    	file_assets = {
    		rs = { "Rust", "https://rustacean.net/more-crabby-things/rustdocs.png" },
    	...
    	git_commit_text = "Committing: %s", -- Format string rendered when committing changes in git (either string or function(filename: string): string)
    	plugin_manager_text = "Using %s plugin manager", -- Format string rendered when managing plugins (either string or function(plugin_manager_name: string): string)
